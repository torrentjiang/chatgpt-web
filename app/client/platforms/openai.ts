...
export class ChatGPTApi implements LLMApi {
  ...
  async chat(options: ChatOptions) {
    ...
    const callback = (callbackdata: any) => {
      ...
      const obj = choices;
      // const jsontx = prettyObject(text);
      // let list = []
      // if (jsontx) {
      //   list.push(jsontx);
      // }
    ...
    let bigModel = new TTSRecorder('5354ab8d', requestPayload.messages ,callback);
    bigModel.start();
    // try {
    //   const chatPath =
    //     "https://demo-dev.saicmobility.com/node-api/aigc/knowledgeV4";
    //   const chatPayload = {
    //     method: "POST",
    ...
    //           if (delta) {
    //             responseText += delta;
    //             options.onUpdate?.(responseText, "", source);
    //             finish();
    //           }
    //         } catch (e) {
